...
def kirjuta_logifaili(p_kommentaar, p_vaartus):
    ...
    rida = praegu.strftime("%d.%m.%Y") + " " + praegu.strftime("%X") + " " + p_kommentaar + ": " + str(p_vaartus)
    fail.write(rida + "\n")
    fail.close()

def tänane_kuupäev():
    # leiab tänase kuupäeva
...
try:
    sisend_kuupäev = input("Sisesta kuupäev (kujul pp.kk.aaaa): ")
    kirjuta_logifaili("Kasutaja sisend", sisend_kuupäev)
    i_päev, i_kuu, i_aasta = map(int, sisend_kuupäev.split('.'))
